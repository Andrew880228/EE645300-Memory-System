...
class CACHE_REPLACEMENT_STATE
{
  ...
  public:
    ...
    INT32  GetVictimInSet( UINT32 tid, UINT32 setIndex, const LINE_REPLACEMENT_STATE *vicSet, UINT32 assoc, Addr_t PC, Addr_t paddr, UINT32 accessType );
    void   UpdateReplacementState( UINT32 setIndex, INT32 updateWayID, const LINE_REPLACEMENT_STATE *currLine, UINT32 tid, Addr_t PC, UINT32 accessType, bool cacheHit );
  ...
  private:
    ...
    void   PrintStat(LINE_REPLACEMENT_STATE *repl, UINT32 way, bool cacheHit, Addr_t pc, int originalSig, bool originalOut);
};
...
void CACHE_REPLACEMENT_STATE::ReadData()
{
    cin>>SHCTnum>>operation;
    
    int vic;
    
    SHCT = new UINT32[SHCTnum];
    ...
    for (int i = 0; i < operation; i++) {
        ...
        if (hit == 1) {
            cin>>hitway;
            cout<<serial<<" "<<pc<<" "<<hitset<<" Hit "<<hitway<<endl;
            UpdateReplacementState(hitset, hitway, repl[hitset], serial, pc, 2, hit);
        } else {
            cout<<serial<<" "<<pc<<" "<<hitset<<" Replace ";
            vic = GetVictimInSet(serial, hitset, repl[hitset], assoc, pc, 0, 2);
            cout<<vic<<endl;
            UpdateReplacementState(hitset, vic, repl[hitset], serial, pc, 2, hit);
        }
        cout<<endl;
    }
}
...
//                                                                            //
// /////////////////////////////////////////////////////////////////////////////
INT32 CACHE_REPLACEMENT_STATE::GetVictimInSet( UINT32 tid, UINT32 setIndex, const LINE_REPLACEMENT_STATE *vicSet, UINT32 assoc, Addr_t PC, Addr_t paddr, UINT32 accessType )
{
...
void CACHE_REPLACEMENT_STATE::UpdateReplacementState( 
    UINT32 setIndex, INT32 updateWayID, const LINE_REPLACEMENT_STATE *currLine,
    UINT32 tid, Addr_t PC, UINT32 accessType, bool cacheHit )
{
...
{
    int originalSig = repl[setIndex][updateWayID].signature;
    bool originalOut = repl[setIndex][updateWayID].outcome;
...
void CACHE_REPLACEMENT_STATE::PrintStat(LINE_REPLACEMENT_STATE *repl, UINT32 way, bool cacheHit, Addr_t pc, int originalSig, bool originalOut)
{
    cout<<"RRPV:      ";
    for (int i = 0; i < assoc; i++) {
        if (i == way) cout<<"("<<repl[i].RRPV<<") ";
        else cout<<repl[i].RRPV<<" ";
    }
    cout<<endl<<"Signature: ";
    for (int i = 0; i < assoc; i++) {
        if (i == way && !cacheHit) cout<<"("<<repl[i].signature<<") ";
        else cout<<repl[i].signature<<" ";
    }
    cout<<endl<<"Outcome:   ";
    for (int i = 0; i < assoc; i++) {
        if (i == way) cout<<"("<<repl[i].outcome<<") ";
        else cout<<repl[i].outcome<<" ";
    }
    cout<<endl<<"SHCT:      ";
    for (int i = 0; i < SHCTnum; i++) {
        if (i == originalSig) {
...
int main()
{
    int set, way, pol, SHCT, operation;
    int serial, pc, hset, hit, hway;
    int i, j;
    cin >> set >> way >> pol;
